...
func main() {
	...
	file = guardedCapture{source: capture}
	pointerAddr = 0
	startSharingGuards()
	...
	publicServer := &http.Server{Handler: publicViewer(eventPublisher), ReadHeaderTimeout: 10 * time.Second}
	defer publicServer.Close()
	probeListener, probeErr := net.Listen("tcp", "127.0.0.1:2002")
	if probeErr != nil {
		log.Fatal(probeErr)
	}
	go publicServer.Serve(probeListener)
	...
	if listenerResult.TailscaleManager != nil {
		go func() {
			...
			case <-tm.Ready():
				...
				if c.PublicViewer {
					publicListener, err := tm.PublicListener()
					if err != nil {
						log.Printf("Public viewer unavailable: %v", err)
					} else {
	...
	case <-serviceShutdown:
		log.Println("Owner requested service power-down; exiting successfully until next device restart")
		cancel()
		server.Close()
		return
	case sig := <-sigChan:
