...
func getFramePointer(pid string) (int64, error) {
	...
	if err != nil || !strings.Contains(strings.ToLower(string(model)), "chiappa") {
		return 0, fmt.Errorf("this build requires Paper Pro Move")
	}
	firmware, err := os.ReadFile("/etc/os-release")
	if err != nil || !strings.Contains(string(firmware), `IMG_VERSION="3.28.0.169"`) {
		return 0, fmt.Errorf("capture only validated on firmware 3.28.0.169")
	}
	maps, err := os.ReadFile(fmt.Sprintf("/proc/%s/maps", pid))
	if err != nil {
		return 0, err
	}
	mem, err := os.Open(fmt.Sprintf("/proc/%s/mem", pid))
	if err != nil {
		return 0, err
	}
	defer mem.Close()
	return locateFrameAllocation(string(maps), mem, ScreenSizeBytes)
}
...
func calculateFramePointer(pid string, startAddress int64) (int64, error) {
	...
	for _, line := range strings.Split(string(maps), "\n") {
		...
		start, _ := strconv.ParseInt(bounds[0], 16, 64)
		if start == startAddress {
			mappingEnd, _ = strconv.ParseInt(bounds[1], 16, 64)
		}
	}
	if mappingEnd <= startAddress {
		return 0, fmt.Errorf("missing bounded display mapping")
	}
	steps := 0
	...
	for length < ScreenSizeBytes {
		steps++
		if steps > 1024 || length < 2 {
			return 0, fmt.Errorf("invalid allocation walk")
		}
		offset += int64(length - 2)
		if offset < 0 || startAddress+offset+16 > mappingEnd {
			return 0, fmt.Errorf("allocation outside mapping")
		}
	...
	if length > ScreenSizeBytes+8192 || startAddress+offset+ScreenSizeBytes > mappingEnd {
		return 0, fmt.Errorf("unexpected frame allocation")
	}
